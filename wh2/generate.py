...
armour_types = read_to_dict(TWDBReader("unit_armour_types_tables"))
...
def ability_phase_details_stats(phase_id, indent=0, title=""):
    ...
    result += indent_str(indent) + title + "[[col:" + col + "]] " + duration + replenish_ammo + unbreakable + mana_regen_mod + mana_max_depletion_mod + cant_move + freeze_fatigue + fatigue_change_ratio + aura_range_mod + ability_recharge_change + recharge_time + "[[/col]]" + endl
    ...
    if int(details["heal_amount"]) != 0:
        ...
        result += indent_str(indent + 2) + "heal each entity " + resurrect + "by " + stat_str(details["heal_amount"]) + " every " + stat_str(details["hp_change_frequency"] + "s") + endl
    ...
    if int(details["damage_amount"]) != 0:
        ...
        result += indent_str(indent + 2) + chance + "damage " + up_to + "entities, each by: " + ability_damage_stat(details["damage_amount"], details["imbue_ignition"], details["imbue_magical"]) + " every " + stat_str(details["hp_change_frequency"] + "s") + endl

    if phase_id in ability_phase_stats:
        result += indent_str(indent + 2) + "stats:" + endl
        ...
        for effect in effects:
            ...
            if how == '+' and float(effect["value"]) < 0:
                how = ""
            result += indent_str(indent + 2) + effect["stat"] + " " + how + stat_str(round(float(effect["value"]), 2)) + endl
...
def named_stat(name, stat, indent=0):
    return indent_str(indent) + name + " " + stat_str(stat) + endl
...
def explosion_stats(explosion_row, indent=0):
    projectile_text = ""
    ...
    projectile_text += indent_str(indent) + damage_stat(explosion_row["detonation_damage"], explosion_row["detonation_damage_ap"], explosion_row["ignition_amount"], explosion_row["is_magical"], "per_entity_dmg") + endl
    projectile_text += named_stat("radius", explosion_row["detonation_radius"], indent)
    if explosion_row["affects_allies"] == "false":
        projectile_text += positive_str("doesn't_affect_allies", indent) + endl
    if explosion_row["shrapnel"]:
        ...
        if shrapnel_row["launch_type"] == "sector":
            projectile_text += named_stat("angle", shrapnel_row["sector_angle"], 2 + indent)
        projectile_text += named_stat("amount", shrapnel_row["amount"], 2 + indent)
        projectile_text += missile_stats(projectile_types[shrapnel_row["projectile"]], None, 2 + indent, False)
    if explosion_row["contact_phase_effect"]:
        projectile_text += ability_phase_details_stats(explosion_row["contact_phase_effect"], 2 + indent, "contact effect")
    return projectile_text


def missile_stats(projectile_row, unit, indent, trajectory=True):
    projectile_text = ""
    # projectile:
    ...
    if projectile_row["can_target_airborne"] == "true":
        building += "@airborne "
    projectile_text += indent_str(indent) + damage_stat(projectile_row["damage"], projectile_row["ap_damage"], projectile_row["ignition_amount"], projectile_row["is_magical"]) + building + endl
    ...
    projectile_text += indent_str(indent) + "calibration: " + "area " + stat_str(projectile_row["calibration_area"]) + " distance " + stat_str(projectile_row["calibration_distance"]) + " prefers " + central_targets + endl

    if volley != "":
        projectile_text += indent_str(indent) + volley + endl
    if unit is not None:
        ...
        projectile_text += indent_str(indent) + "reload: " + "skill " + stat_str(unit["reload"]) + " time " + stat_str(reload_time) + "s (base" + stat_str(projectile_row["base_reload_time"]) + "s)" + endl

    category = projectile_row["category"]
    if category == "misc" or category == "artillery":
        category += "(ignores shields)"
    projectile_text += indent_str(indent) + "category: " + stat_str(category) + " spin " + stat_str(projectile_row["spin_type"].replace("_spin", "", 1)) + endl
    ...
    if trajectory:
        ...
        projectile_text += indent_str(indent) + trajectory + endl
    ...
    if projectile_row["explosion_type"] != "":
        ...
        projectile_text += explosion_stats(explosion_row, indent + 2)
    return projectile_text
...
def melee_weapon_stats(melee_id, indent=0):
    ...
    unit_desc += indent_str(indent) + damage_stat(melee_row["damage"], melee_row["ap_damage"], melee_row["ignition_amount"], melee_row["is_magical"], "melee_dmg") + building + endl
    ...
    unit_desc += indent_str(indent) + "melee_interval " + stat_str(melee_row["melee_attack_interval"]) + " dp10s " + derived_stat_str(round(dp10s, 0)) + endl
    ...
    if melee_row["splash_attack_target_size"] != "":
        ...
        unit_desc += indent_str(indent + 2) + "max_targets " + stat_str(melee_row["splash_attack_max_attacks"]) + " dmg_each " + derived_stat_str(round(total_dmg / float(melee_row["splash_attack_max_attacks"]), 0)) + endl
    ...
    if melee_row["collision_attack_max_targets"] != "0":
        unit_desc += indent_str(indent) + " collision: max targets " + stat_str(melee_row["collision_attack_max_targets"]) + " recharge_per_sec " + stat_str(melee_row["collision_attack_max_targets_cooldown"]) + endl
    return unit_desc


def missile_weapon_stats(missile_weapon, unit, title="ranged", indent=0):
    projectile_text = ""
    projectile_id = weapon_projectile[missile_weapon]
    name = ""
    if weapon_secondary_ammo[missile_weapon] == "true":
        name = "(secondary ammo)"
    projectile_text += indent_str(indent) + title + name + ":" + endl
    projectile_row = projectile_types[projectile_id]
    projectile_text += missile_stats(projectile_row, unit, indent + 2)
    if missile_weapon in weapon_alt_projectile:
        for alt_projectile_id in weapon_alt_projectile[missile_weapon]:
            ...
            if weapon_secondary_ammo[missile_weapon] == "true":
                name += "(secondary ammo)"
            projectile_text += indent_str(indent) + title + " (" + name + "):" + endl
            projectile_text += missile_stats(alt_projectile_row, unit, indent + 2)
    return projectile_text
...
ability_details_proto_map = {"key": "ikit_claw_missile_tooltip", "num_uses": "-1", "active_time": "-1", "recharge_time": "-1", "initial_recharge": "-1", "wind_up_time": "0", "passive": "true", "effect_range": "0", "affect_self": "false", "num_effected_friendly_units": "0", "num_effected_enemy_units": "0", "update_targets_every_frame": "0", "target_friends": "false", "target_enemies": "false", "target_ground": "false", "target_intercept_range": "0", "clear_current_order": "false", "unique_id": "17224802351", "mana_cost": "0", "min_range": "0", "miscast_chance": "0", "voiceover_state": "vo_battle_special_ability_generic_response", "additional_melee_cp": "0", "additional_missile_cp": "0", "target_ground_under_allies": "false", "target_ground_under_enemies": "false", "miscast_global_bonus": "false", "target_self": "true", "spawn_is_transformation": "false", "use_loop_stance": "false", "spawn_is_decoy": "false", "only_affect_owned_units": "false", "shared_recharge_time": "-1"}
...
for effect_id in effect_bonus_missile_junctions:
    ...
    for effect_row in effect_rows:
        ...
        ability_text_row["text"] = missile_weapon_stats(weapon_id, None)
...
with TWDBReader("main_units_tables") as db_reader:
    for row in db_reader.rows_iter:
        ...
        if main_unit_entry["is_high_threat"] == "true":
            unit_desc += stat_str("high_threat (focuses enemy attack and splash damage)") + endl
        ...
        unit_desc += indent_str(indent) + "run_speed " + stat_str(speed) + " charge " + stat_str(charge_speed) + " acceleration " + stat_str(accel * 10) + endl
        if fly_speed != 0:
            unit_desc += indent_str(indent) + "fly_speed " + stat_str(fly_speed) + " charge " + stat_str(fly_charge_speed) + endl
        ...
        if unit["ground_stat_effect_group"] != "" and unit["ground_stat_effect_group"] in ground_type_stats:
            ...
            for gtype in ground_types:
                ...
                unit_desc += indent_str(indent + 2) + stat_desc + endl

        if unit["armour"] != "":
            armour_id = unit["armour"]
            armour_row = armour_types[armour_id]
        ...
        if main_unit_entry["unit"] in missile_weapon_junctions:
            unit_desc += stat_str("ranged_weapon_replacement_available_in_campaign [[img:ui/battle ui/ability_icons/ranged_weapon_stat.png]][[/img]]") + endl

        if len(ranged_weapons_set) > 1:
            print("missile weapon conflict (land unit):" + unit["key"])
        for missile_weapon in ranged_weapons_set:
            unit_desc += missile_weapon_stats(missile_weapon, unit, indent=indent)
        ...
        for missile_id in support_ranged_weapons:
            unit_desc += missile_weapon_stats(missile_id, None, "ranged_support")

        spawn_info = unit_name[main_unit_entry["land_unit"]] + " (" + main_unit_entry["caste"] + ", tier " + main_unit_entry["tier"] + " men " + num_str(num_men) + ")"
            ...
            if description_id in ability_details:
                ...
                if ability["vortex"] != "":
                    ...
                    if vortex["num_vortexes"] != "1":
                        result += indent_str(indent) + " vortex count: " + stat_str(vortex["num_vortexes"]) + " vortexes " + stat_str(vortex["delay_between_vortexes"]) + "s delay between" + endl
                    radius = ""
                    ...
                    result += indent_str(indent) + "radius: " + radius + endl
                    result += indent_str(indent) + damage_stat(vortex["damage"], vortex["damage_ap"], vortex["ignition_amount"], vortex["is_magical"]) + endl
                    ...
                    if vortex["building_collision"] == "2.expire":
                        result += indent_str(indent) + stat_str("building collision expires vortex") + endl
                    ...
                    result += indent_str(indent) + "path: " + path + endl
                    if vortex["affects_allies"] == "false":
                        result += positive_str("doesn't_affect_allies", indent) + endl
                ...
                if ability["miscast_explosion"] != "":
                    ...
                    result += explosion_stats(explosion_row, 2)
                    result += endl
...
stat_icon = {"armour": icon("icon_stat_armour"), "melee_damage_ap": icon("modifier_icon_armour_piercing"), "fatigue": icon("fatigue"), "accuracy": "accuracy", "morale": icon("icon_stat_morale"), "charging": icon("icon_stat_charge_bonus"), "charge_bonus": icon("icon_stat_charge_bonus"), "range": icon("icon_stat_range"), "speed": icon("icon_stat_speed"), "reloading": icon("icon_stat_reload_time"), "melee_attack": icon("icon_stat_attack"), "melee_defence": icon("icon_stat_defence")}
        ...
        if key == "unit_stat_localisations_tooltip_text_scalar_speed":
            ...
            for fatigue_level in fatigue_order:
                ...
                for stat in fatigue_effects[fatigue_level]:
                    new_text += " " + stat_icon[stat] + "" + stat_str(float(fatigue_effects[fatigue_level][stat]) * 100) + "%"
                new_text += '||'
    ...
    for new_row in db_reader.rows_iter:
        ...
        if key == "uied_component_texts_localised_string_experience_tx_Tooltip_5c0016":
            ...
            for rank in range(1, 10):
                ...
                for stat in stats:
                    new_text += stat_icon[stat] + " " + stats[stat] + " "
                new_text += '||'
        new_row["text"] += new_text
    db_writer.write()
